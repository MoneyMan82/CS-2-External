...
namespace External_Aimbot
{
    internal static class Visibility
    {
        ...
        private const float CrosshairSlackDegrees = 5f;
            ...
            bool useMapRaytracing)
        {
            ...
            if (useMapRaytracing && MapCollision.IsLoaded)
                return MapCollision.IsVisible(localEye, targetAim, targetChest);

            if (useMapRaytracing && !MapCollision.IsLoaded)
                return IsWithinAimCone(localEye, targetAim, currentViewAngles, aimFov);
            ...
            if (Vector3.Dot(viewDir, toTarget) < MinForwardDot)
                return false;

            if (IsSpottedByLocalPlayer(mem, targetPawn, localControllerIndex))
                return true;

            return IsWithinAimCone(localEye, targetAim, currentViewAngles, aimFov);
        }

        private static bool IsWithinAimCone(
            Vector3 localEye,
            Vector3 targetAim,
            Vector2 currentViewAngles,
            float aimFov)
        {
            float angleToTarget = Calculate.GetFovDistance(
                currentViewAngles,
                Calculate.CalculateAngles(localEye, targetAim));

            float crosshairSlack = Math.Clamp(aimFov * 0.35f, CrosshairSlackDegrees, aimFov);
            return angleToTarget <= crosshairSlack;
        }
